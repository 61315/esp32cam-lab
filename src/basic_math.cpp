...
static const char* TAG = "example:basic_math";
...
static void process_and_show(float* data, int length) {
    ...
    for (int i = 0; i < length / 2; i++) {
        data[i] =
            10 *
            log10f((data[i * 2 + 0] * data[i * 2 + 0] + data[i * 2 + 1] * data[i * 2 + 1]) / N);
    }
...
void setup() {
    ...
    dsps_mul_f32(x1, wind, y_cf, N, 1, 1,
                 2); // Multiply input array with window and store as real part
